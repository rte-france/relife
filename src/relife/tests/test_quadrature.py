...
def test_legendre_quadrature(integration_bound_a, integration_bound_b):
    ...
    assert integration == approx(
        expected_intg(integration_bound_a, integration_bound_b)
    )
    ...
    assert integration.shape == (d,) + np.broadcast_shapes(
        integration_bound_a.shape, integration_bound_b.shape
    )
    assert integration == approx(
        stack_f(expected_intg(integration_bound_a, integration_bound_b))
    )
