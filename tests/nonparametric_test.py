...
def test_fit_ecdf_kaplan_meier(data):
    ...
    )  # mais tester l'estimate avec qd mm pour verifier que tt fonctionne shape wise
...
def test_turnbull(data_turnbull, weibull_model):
    ...
    """
    print(data_turnbull[:-1])
    tb = Turnbull(data_turnbull[:-1], entry=data_turnbull[-1], lowmem=True)
    t = tb.timeline / 6
