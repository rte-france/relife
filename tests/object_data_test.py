...
def example_1d_data():
    return {
        "censored_lifetimes": np.array([10, 11, 9, 10, 12, 13, 11]),
...
def test_1d_data(example_1d_data):
    observed = ObservedFromIndicators(
        example_1d_data["censored_lifetimes"], example_1d_data["event"] == 1
    )
    ...
    assert (observed.index == np.array([0, 2, 6])).all()
    assert (observed.values == np.array([10, 9, 11])).all()
...
def test_1d_data_factory(example_1d_data):
    ...
    left_censored = left_censored_factory(example_1d_data["censored_lifetimes"])
    right_censored = right_censored_factory(
        example_1d_data["censored_lifetimes"], example_1d_data["event"] == 0
    )
    interval_censored = interval_censored_factory(example_1d_data["censored_lifetimes"])
    ...
    assert interval_truncated.values.size == 0 and interval_truncated.values.shape == (
        0,
        2,
    )
...
def test_2d_data(example_2d_data):
    observed = Observed(example_2d_data["censored_lifetimes"])
    ...
    assert (interval_censored.values == np.array([[1, 2], [2, 10], [10, 11]])).all()
...
def test_2d_data_factory(example_2d_data):
    observed = observed_factory(example_2d_data["censored_lifetimes"])
    left_censored = left_censored_factory(example_2d_data["censored_lifetimes"])
    right_censored = right_censored_factory(example_2d_data["censored_lifetimes"])
    interval_censored = interval_censored_factory(example_2d_data["censored_lifetimes"])
