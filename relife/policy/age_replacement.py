...
from relife.lifetime_model import AgeReplacementModel, FrozenLeftTruncatedModel, FrozenAgeReplacementModel, FrozenLifetimeRegression, LifetimeDistribution
...
class AgeReplacementPolicy(AgeRenewalPolicy):
    ...
    def optimize(
        ...
        if self.ar1 is not None:
            one_cycle_ar_policy = OneCycleAgeReplacementPolicy(self.lifetime_model, self.cf, self.cp, self.discounting).optimize()
