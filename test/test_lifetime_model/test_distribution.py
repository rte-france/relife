...
def test_isf(distribution, probability, expected_out_shape):
    ...
    assert distribution.isf(np.full(expected_out_shape(probability=probability), 0.5)) == approx(np.full(expected_out_shape(probability=probability), distribution.median()))
