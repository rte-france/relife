...
def test_ls_integrate(regression, integration_bounds, covar):
    m = 2
    # integral_a^b dF(x)
    a, b = integration_bounds
    z = covar(m, regression.nb_coef)
    shape = np.broadcast_shapes(a.shape, b.shape, (z.shape[0], 1))
    ...
    integration = regression.ls_integrate( lambda x: x, np.zeros_like(a), np.full_like(b, np.inf), covar(m, regression.nb_coef), deg=100)
    assert integration == approx(np.full(shape, regression.mean(covar(m, regression.nb_coef))), rel=1e-3)
