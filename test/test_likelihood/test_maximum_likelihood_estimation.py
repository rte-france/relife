...
def test_distribution(distribution, power_transformer_data):
    ...
    fitted_distribution = maximum_likelihood_estimation(
        type(distribution)(), LifetimeData(time, event=event, entry=entry)
    )
...
def test_regression(regression, insulator_string_data):
    ...
    type(regression)(baseline).fit(time, covar, event=event)
