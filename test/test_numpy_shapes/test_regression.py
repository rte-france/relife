...
def test_proportional_hazard(proportional_hazard, time, covar, probability):
    ...
    assert proportional_hazard.ppf(probability((n,)), covar(m)).shape  == (m,n)
    assert proportional_hazard.ichf(probability((n,)), covar(m)).shape  == (m,n)
...
def test_aft(aft, time, covar, probability):
    ...
    assert aft.ppf(probability((n,)), covar(m)).shape  == (m,n)
    assert aft.ichf(probability((n,)), covar(m)).shape  == (m,n)
